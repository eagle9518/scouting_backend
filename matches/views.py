...
def match_summaries(request, quantifier, match_number, team_number):
    if not Matches.objects.filter(quantifier=quantifier, match_number=match_number).exists():
        create_matches()
    match = Matches.objects.get(quantifier=quantifier, match_number=match_number)
    all_teams_in_match = [match.red_one, match.red_two, match.red_three, match.blue_one, match.blue_two,
                          match.blue_three]

    team, created = Teams.objects.get_or_create(team_number=team_number)
    team_match_data = Team_Match_Data.objects.get(team=team, quantifier=quantifier, match_number=match_number)
    ...
    return render(request, 'match_summaries.html',
                  {"match": match, "all_teams_in_match": all_teams_in_match, "team_number": team_number,
                   "scoring_grid": scoring_grid, "team_match_data": team_match_data})


def dashboard(request, quantifier, match_number, alliance):
